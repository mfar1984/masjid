...
<head>
    ...
    <title>Edit Kerja Penyelenggaraan - E-Masjid</title>
    <x-favicon />
            ...
            <div class="bg-white shadow-lg border-x border-gray-200 p-6">
                <div class="mb-6">
                    <div class="flex items-center space-x-2 mb-2">
                        ...
                        <h1 class="text-xl font-bold text-gray-900">Edit Kerja Penyelenggaraan</h1>
                    </div>
                    <p class="text-xs text-gray-600">{{ $kerjaPenyelenggaraan->no_kerja }}</p>
                </div>

                <form action="{{ route('kerja-penyelenggaraan.update', $kerjaPenyelenggaraan) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-4">
                            ...
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Jadual Penyelenggaraan</label>
                                <select name="jadual_penyelenggaraan_id" class="w-full px-3 py-2 border border-gray-300 rounded text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    ...
                                    @foreach($jadualPenyelenggaraan as $jadual)
                                        <option value="{{ $jadual->id }}" {{ old('jadual_penyelenggaraan_id', $kerjaPenyelenggaraan->jadual_penyelenggaraan_id) == $jadual->id ? 'selected' : '' }}>{{ $jadual->nama_jadual }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="jenis_item" id="jenisItem" required onchange="toggleItemDropdown()"
                                    ...
                                    <option value="Aset" {{ old('jenis_item', $kerjaPenyelenggaraan->jenis_item) == 'Aset' ? 'selected' : '' }}>Aset</option>
                                    <option value="Fasiliti" {{ old('jenis_item', $kerjaPenyelenggaraan->jenis_item) == 'Fasiliti' ? 'selected' : '' }}>Fasiliti</option>
                                </select>
                            </div>
                            ...
                            <div id="asetDropdown">
                                ...
                                <select name="senarai_aset_id" class="w-full px-3 py-2 border border-gray-300 rounded text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    ...
                                    @foreach($senariAset as $aset)
                                        <option value="{{ $aset->id }}" {{ old('senarai_aset_id', $kerjaPenyelenggaraan->senarai_aset_id) == $aset->id ? 'selected' : '' }}>{{ $aset->nama_aset }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div id="fasilitiDropdown" style="display: none;">
                                ...
                                <select name="senarai_fasiliti_id" class="w-full px-3 py-2 border border-gray-300 rounded text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    ...
                                    @foreach($senariFasiliti as $fasiliti)
                                        <option value="{{ $fasiliti->id }}" {{ old('senarai_fasiliti_id', $kerjaPenyelenggaraan->senarai_fasiliti_id) == $fasiliti->id ? 'selected' : '' }}>{{ $fasiliti->nama_fasiliti }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="jenis_kerja" required class="w-full px-3 py-2 border border-gray-300 rounded text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    @foreach(['Penyelenggaraan Berkala', 'Pembaikan', 'Pemeriksaan', 'Servis', 'Kecemasan'] as $jenis)
                                        <option value="{{ $jenis }}" {{ old('jenis_kerja', $kerjaPenyelenggaraan->jenis_kerja) == $jenis ? 'selected' : '' }}>{{ $jenis }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" name="tarikh_kerja" value="{{ old('tarikh_kerja', $kerjaPenyelenggaraan->tarikh_kerja->format('Y-m-d')) }}" required
                                    class="w-full px-3 py-2 border border-gray-300 rounded text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="time" name="masa_mula" value="{{ old('masa_mula', $kerjaPenyelenggaraan->masa_mula) }}"
                                        class="w-full px-3 py-2 border border-gray-300 rounded text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-1">Masa Tamat</label>
                                    <input type="time" name="masa_tamat" value="{{ old('masa_tamat', $kerjaPenyelenggaraan->masa_tamat) }}"
                                        class="w-full px-3 py-2 border border-gray-300 rounded text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="penerangan_kerja" rows="3" required class="w-full px-3 py-2 border border-gray-300 rounded text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('penerangan_kerja', $kerjaPenyelenggaraan->penerangan_kerja) }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="vendor_nama" value="{{ old('vendor_nama', $kerjaPenyelenggaraan->vendor_nama) }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="vendor_telefon" value="{{ old('vendor_telefon', $kerjaPenyelenggaraan->vendor_telefon) }}"
                                        class="w-full px-3 py-2 border border-gray-300 rounded text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-1">Kos (RM)</label>
                                    <input type="number" name="kos" value="{{ old('kos', $kerjaPenyelenggaraan->kos) }}" step="0.01" min="0"
                                        class="w-full px-3 py-2 border border-gray-300 rounded text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="vendor_alamat" value="{{ old('vendor_alamat', $kerjaPenyelenggaraan->vendor_alamat) }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                                ...
                                <div>
                                    ...
                                    <select name="kondisi_sebelum" class="w-full px-3 py-2 border border-gray-300 rounded text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
                                        ...
                                        @foreach(['Baik', 'Sederhana', 'Teruk', 'Rosak'] as $kondisi)
                                            <option value="{{ $kondisi }}" {{ old('kondisi_sebelum', $kerjaPenyelenggaraan->kondisi_sebelum) == $kondisi ? 'selected' : '' }}>{{ $kondisi }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
                                    ...
                                    <select name="kondisi_selepas" class="w-full px-3 py-2 border border-gray-300 rounded text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
                                        <option value="">-- Pilih --</option>
                                        @foreach(['Baik', 'Sederhana', 'Teruk', 'Rosak'] as $kondisi)
                                            <option value="{{ $kondisi }}" {{ old('kondisi_selepas', $kerjaPenyelenggaraan->kondisi_selepas) == $kondisi ? 'selected' : '' }}>{{ $kondisi }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="status" required class="w-full px-3 py-2 border border-gray-300 rounded text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    @foreach(['Dirancang', 'Sedang Berjalan', 'Selesai', 'Dibatalkan', 'Tertangguh'] as $status)
                                        <option value="{{ $status }}" {{ old('status', $kerjaPenyelenggaraan->status) == $status ? 'selected' : '' }}>{{ $status }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="catatan" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded text-xs focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('catatan', $kerjaPenyelenggaraan->catatan) }}</textarea>
                            </div>
                        </div>
                    </div>
