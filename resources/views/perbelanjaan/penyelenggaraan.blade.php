...
<head>
    ...
    <title>Penyelenggaraan - E-Masjid</title>
    <x-favicon />
        ...
        <div class="container mx-auto px-0 py-0">
            <div class="bg-white shadow-lg border-x border-gray-200 p-6">
                <div class="mb-6 flex items-center justify-between">
                    <div>
                        <h1 class="text-xl font-bold text-gray-900 mb-1">Penyelenggaraan</h1>
                        <p class="text-xs text-gray-600">Rekod perbelanjaan penyelenggaraan</p>
                    </div>
                ...
                <form method="POST" action="{{ route('perbelanjaan.store') }}" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="jenis_perbelanjaan" value="Penyelenggaraan">

                    <div class="bg-green-50 rounded-lg p-4 mb-6">
                        <h2 class="text-sm font-semibold text-gray-900 mb-4">Maklumat Penyelenggaraan</h2>
                        
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label for="jenis_kerja" class="block text-xs font-medium text-gray-700 mb-2">Jenis Kerja *</label>
                                <select id="jenis_kerja" name="jenis_kerja" required class="w-full px-3 py-2 border border-gray-300 rounded-sm text-xs">
                                    <option value="">-- Pilih Jenis --</option>
                                    <option value="Baik Pulih Bangunan">Baik Pulih Bangunan</option>
                                    <option value="Kerja Elektrik">Kerja Elektrik</option>
                                    <option value="Kerja Paip">Kerja Paip</option>
                                    <option value="Pengecatan">Pengecatan</option>
                                    <option value="Pembersihan">Pembersihan</option>
                                    <option value="Lain-lain">Lain-lain</option>
                                </select>
                                @error('jenis_kerja')<span class="text-xs text-red-600">{{ $message }}</span>@enderror
                            </div>

                            <div>
                                <label for="nama_kontraktor" class="block text-xs font-medium text-gray-700 mb-2">Nama Kontraktor</label>
                                <input type="text" id="nama_kontraktor" name="nama_kontraktor" value="{{ old('nama_kontraktor') }}" maxlength="255" class="w-full px-3 py-2 border border-gray-300 rounded-sm text-xs">
                                @error('nama_kontraktor')<span class="text-xs text-red-600">{{ $message }}</span>@enderror
                            </div>

                            <div>
                                <label for="tarikh_perbelanjaan" class="block text-xs font-medium text-gray-700 mb-2">Tarikh Kerja *</label>
                    ...
                    <div class="bg-green-50 rounded-lg p-4 mb-6">
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            <div>
                                <label for="keterangan" class="block text-xs font-medium text-gray-700 mb-2">Keterangan Kerja *</label>
                    ...
                    <div class="flex items-center justify-end space-x-3 pt-4 border-t border-gray-200">
                        ...
                        <button type="submit" class="px-4 py-2 bg-green-600 text-white text-xs rounded hover:bg-green-700">Simpan Perbelanjaan</button>
                    </div>
                </form>
            </div>
        </div>
