...
<head>
    ...
    <title>Lihat Kelulusan Pelupusan - E-Masjid</title>
    <x-favicon />
            ...
            <div class="bg-white shadow-lg border-x border-gray-200 p-6">
                <div class="mb-6 flex items-center justify-between">
                    <div class="flex items-center space-x-2">
                        <a href="{{ route('kelulusan-pelupusan.index') }}" class="text-blue-600 hover:text-blue-800">
                            <span class="material-icons" style="font-size: 20px !important;">arrow_back</span>
                        </a>
                        <div>
                            <h1 class="text-xl font-bold text-gray-900">Maklumat Kelulusan Pelupusan</h1>
                            <p class="text-xs text-gray-600">{{ $permohonanPelupusan->no_rujukan }}</p>
                        </div>
                    </div>
                    @if($permohonanPelupusan->status === 'Menunggu' && auth()->user()->hasPermission('kelulusan_pelupusan', 'approve'))
                        <div class="flex space-x-2">
                            <form action="{{ route('kelulusan-pelupusan.approve', $permohonanPelupusan) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" class="inline-flex items-center px-4 py-2 bg-green-600 text-white text-xs rounded hover:bg-green-700">
                                    <span class="material-icons mr-1" style="font-size: 16px !important;">check_circle</span>
                                    Luluskan
                                </button>
                            </form>
                        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="bg-gray-50 p-4 rounded border border-gray-200 md:col-span-2">
                        <h3 class="text-sm font-semibold text-gray-900 mb-3">Sebab Pelupusan</h3>
