...
<head>
    ...
    <title>Edit Jadual Program - E-Masjid</title>
    <x-favicon />
        ...
        <div class="container mx-auto px-0 py-0">
            <div class="bg-white shadow-lg border-x border-gray-200 p-6">
                <div class="flex items-center mb-6">
                    ...
                    <div>
                        <h1 class="text-xl font-bold text-gray-900 mb-1">Edit Jadual Program</h1>
                        <p class="text-xs text-gray-600">{{ $jadualProgram->program->nama_program ?? '-' }}</p>
                    </div>
                </div>
                <form action="{{ route('jadual-program.update', $jadualProgram) }}" method="POST">
                    @csrf
                    @method('PUT')
                        ...
                        <div>
                            ...
                            <select name="program_id" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                                @foreach($programList as $program)
                                    <option value="{{ $program->id }}" {{ old('program_id', $jadualProgram->program_id) == $program->id ? 'selected' : '' }}>{{ $program->nama_program }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-700 mb-1">Tarikh <span class="text-red-500">*</span></label>
                            <input type="date" name="tarikh" value="{{ old('tarikh', $jadualProgram->tarikh->format('Y-m-d')) }}" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                        </div>
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Masa Mula <span class="text-red-500">*</span></label>
                                <input type="time" name="masa_mula" value="{{ old('masa_mula', $jadualProgram->masa_mula) }}" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Masa Tamat <span class="text-red-500">*</span></label>
                                <input type="time" name="masa_tamat" value="{{ old('masa_tamat', $jadualProgram->masa_tamat) }}" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                            </div>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-700 mb-1">Lokasi</label>
                            <input type="text" name="lokasi" value="{{ old('lokasi', $jadualProgram->lokasi) }}" class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-700 mb-1">Penceramah</label>
                            <input type="text" name="penceramah" value="{{ old('penceramah', $jadualProgram->penceramah) }}" class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                        </div>
                        <div>
                            ...
                            <select name="status" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                                @foreach(['Dijadual', 'Sedang Berlangsung', 'Selesai', 'Batal'] as $s)
                                    <option value="{{ $s }}" {{ old('status', $jadualProgram->status) === $s ? 'selected' : '' }}>{{ $s }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-700 mb-1">Catatan</label>
                            <textarea name="catatan" rows="2" class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">{{ old('catatan', $jadualProgram->catatan) }}</textarea>
                        </div>
                    </div>
