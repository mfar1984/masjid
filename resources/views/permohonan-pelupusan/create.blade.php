...
<head>
    ...
    <title>Tambah Permohonan Pelupusan - E-Masjid</title>
    <x-favicon />
            ...
            <div class="bg-white shadow-lg border-x border-gray-200 p-6">
                <div class="mb-6 flex items-center justify-between">
                    <div class="flex items-center space-x-2">
                        ...
                        <div>
                            <h1 class="text-xl font-bold text-gray-900">Tambah Permohonan Pelupusan</h1>
                            <p class="text-xs text-gray-600">Mohon untuk melupuskan aset</p>
                        </div>
                    </div>
                </div>

                <form action="{{ route('permohonan-pelupusan.store') }}" method="POST">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <select name="senarai_aset_id" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500">
                                ...
                                @foreach($senariAset as $aset)
                                    <option value="{{ $aset->id }}" {{ old('senarai_aset_id') == $aset->id ? 'selected' : '' }}>
                                        {{ $aset->no_siri }} - {{ $aset->nama_aset }} ({{ $aset->status_aset }})
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="date" name="tarikh_permohonan" value="{{ old('tarikh_permohonan', date('Y-m-d')) }}" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500">
                        ...
                        <div>
                            ...
                            <select name="kaedah_pelupusan" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500">
                                <option value="Jualan" {{ old('kaedah_pelupusan') == 'Jualan' ? 'selected' : '' }}>Jualan</option>
                                <option value="Derma" {{ old('kaedah_pelupusan') == 'Derma' ? 'selected' : '' }}>Derma</option>
                                <option value="Buang" {{ old('kaedah_pelupusan') == 'Buang' ? 'selected' : '' }}>Buang</option>
                                <option value="Tukar Ganti" {{ old('kaedah_pelupusan') == 'Tukar Ganti' ? 'selected' : '' }}>Tukar Ganti</option>
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" name="nilai_pelupusan" step="0.01" min="0" value="{{ old('nilai_pelupusan', 0) }}" class="w-full px-3 py-2 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="sebab_pelupusan" rows="3" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500">{{ old('sebab_pelupusan') }}</textarea>
                            @error('sebab_pelupusan')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                        </div>

                        <div class="md:col-span-2">
                            <label class="block text-xs font-medium text-gray-700 mb-1">Catatan</label>
                            <textarea name="catatan" rows="2" class="w-full px-3 py-2 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500">{{ old('catatan') }}</textarea>
