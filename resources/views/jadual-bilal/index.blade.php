...
<head>
    ...
    <title>Jadual Bilal - E-Masjid</title>
    <x-favicon />
        ...
        <div class="container mx-auto px-0 py-0">
            <div class="bg-white shadow-lg border-x border-gray-200 p-6">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
                    <div>
                        <h1 class="text-xl font-bold text-gray-900 mb-1">Jadual Bilal</h1>
                        <p class="text-xs text-gray-600">Pengurusan jadual tugas bilal</p>
                    </div>
                    <div class="mt-4 md:mt-0">
                        <a href="{{ route('jadual-bilal.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-xs font-medium rounded-md hover:bg-blue-700">
                            <span class="material-icons text-sm mr-1">add</span>Tambah Jadual
                        </a>
                    </div>
                </div>
                <x-statistics-grid :stats="$stats" />
                <form method="GET" action="{{ route('jadual-bilal.index') }}" class="mb-4">
                    <div class="flex flex-col md:flex-row gap-3 items-stretch md:items-center">
                        <x-search-input name="search" :value="request('search')" placeholder="Cari nama bilal..." />
                        ...
                        <div class="flex gap-2">
                            ...
                            <x-action-button type="button" icon="refresh" color="red" onclick="window.location.href='{{ route('jadual-bilal.index') }}'">Reset</x-action-button>
                        </div>
                    </div>
                </form>
                    ...
                    <table class="min-w-full text-left text-sm">
                        <thead class="bg-blue-100 text-gray-600">
                            <tr>
                                ...
                                <th class="px-4 py-2 table-header">Nama Bilal</th>
                            ...
                            @forelse($jadualList as $jadual)
                                <tr class="hover:bg-white">
                                    ...
                                    <td class="px-4 py-2 table-data">
                                        <span class="inline-flex items-center px-2 py-1 rounded-sm text-xs font-medium bg-green-100 text-green-800">{{ $jadual->waktu_solat }}</span>
                                    </td>
                                    <td class="px-4 py-2 table-data text-gray-900">{{ $jadual->nama_bilal ?? ($jadual->ajk->nama ?? '-') }}</td>
                                    ...
                                    <td class="px-4 py-2 table-data text-center">
                                        <x-action-icons 
                                            :showUrl="route('jadual-bilal.show', $jadual)"
                                            :editUrl="route('jadual-bilal.edit', $jadual)"
                                            :deleteUrl="route('jadual-bilal.destroy', $jadual)"
                                        />
                                    </td>
                                </tr>
                            @empty
                                <tr><td colspan="6" class="px-4 py-8 text-center text-gray-500"><span class="material-icons mb-2" style="font-size: 48px !important;">campaign</span><p class="text-sm">Tiada jadual dijumpai</p></td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                <div class="md:hidden space-y-3">
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                        <div class="flex items-center justify-between mb-3">
                            <div class="flex-1">
                                <h3 class="mobile-title text-gray-900">{{ $jadual->nama_bilal ?? ($jadual->ajk->nama ?? '-') }}</h3>
                        ...
                        <div class="flex items-center justify-end">
                            <x-action-icons :showUrl="route('jadual-bilal.show', $jadual)" :editUrl="route('jadual-bilal.edit', $jadual)" :deleteUrl="route('jadual-bilal.destroy', $jadual)" />
                        </div>
                    </div>
                    @empty
                    <div class="text-center py-8"><span class="material-icons mb-2 text-gray-400" style="font-size: 48px !important;">campaign</span><p class="text-sm text-gray-500">Tiada jadual dijumpai</p></div>
                    @endforelse
                </div>
