...
<head>
    ...
    <title>Edit Jadual Penyusutan - E-Masjid</title>
    <x-favicon />
            ...
            <div class="bg-white shadow-lg border-x border-gray-200 p-6">
                <div class="mb-6 flex items-center justify-between">
                    <div class="flex items-center space-x-2">
                        ...
                        <div>
                            <h1 class="text-xl font-bold text-gray-900">Edit Jadual Penyusutan</h1>
                            <p class="text-xs text-gray-600">{{ $jadualPenyusutan->kategoriAset->nama_kategori ?? '-' }}</p>
                        </div>
                    </div>
                </div>

                <form action="{{ route('jadual-penyusutan.update', $jadualPenyusutan) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="kategori_aset_id" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500">
                                @foreach($kategoriAset as $kategori)
                                    <option value="{{ $kategori->id }}" {{ old('kategori_aset_id', $jadualPenyusutan->kategori_aset_id) == $kategori->id ? 'selected' : '' }}>{{ $kategori->nama_kategori }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" name="kadar_susut_tahunan" step="0.01" min="0" max="100" value="{{ old('kadar_susut_tahunan', $jadualPenyusutan->kadar_susut_tahunan) }}" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500">
                        ...
                        <div>
                            ...
                            <select name="kaedah_susut" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500">
                                <option value="Garis Lurus" {{ old('kaedah_susut', $jadualPenyusutan->kaedah_susut) == 'Garis Lurus' ? 'selected' : '' }}>Garis Lurus</option>
                                <option value="Baki Berkurangan" {{ old('kaedah_susut', $jadualPenyusutan->kaedah_susut) == 'Baki Berkurangan' ? 'selected' : '' }}>Baki Berkurangan</option>
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" name="tempoh_guna_tahun" min="1" max="50" value="{{ old('tempoh_guna_tahun', $jadualPenyusutan->tempoh_guna_tahun) }}" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500">
                        ...
                        <div>
                            ...
                            <select name="status" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500">
                                <option value="Aktif" {{ old('status', $jadualPenyusutan->status) == 'Aktif' ? 'selected' : '' }}>Aktif</option>
                                <option value="Tidak Aktif" {{ old('status', $jadualPenyusutan->status) == 'Tidak Aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                            </select>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="catatan" rows="3" class="w-full px-3 py-2 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500">{{ old('catatan', $jadualPenyusutan->catatan) }}</textarea>
