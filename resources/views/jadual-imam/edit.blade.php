...
<head>
    ...
    <title>Edit Jadual Imam - E-Masjid</title>
    <x-favicon />
        ...
        <div class="container mx-auto px-0 py-0">
            <div class="bg-white shadow-lg border-x border-gray-200 p-6">
                <div class="flex items-center mb-6">
                    ...
                    <div>
                        <h1 class="text-xl font-bold text-gray-900 mb-1">Edit Jadual Imam</h1>
                        <p class="text-xs text-gray-600">{{ $jadualImam->nama_imam ?? '-' }}</p>
                    </div>
                </div>
                <form action="{{ route('jadual-imam.update', $jadualImam) }}" method="POST">
                    @csrf
                    @method('PUT')
                            ...
                            <div>
                                ...
                                <select name="ajk_id" id="ajk_id" class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                                    ...
                                    @foreach($ajkList as $ajk)
                                        <option value="{{ $ajk->id }}" data-nama="{{ $ajk->nama }}" {{ old('ajk_id', $jadualImam->ajk_id) == $ajk->id ? 'selected' : '' }}>{{ $ajk->nama }} - {{ $ajk->jawatan }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Nama Imam <span class="text-red-500">*</span></label>
                                <input type="text" name="nama_imam" id="nama_imam" value="{{ old('nama_imam', $jadualImam->nama_imam) }}" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Tarikh <span class="text-red-500">*</span></label>
                                <input type="date" name="tarikh" value="{{ old('tarikh', $jadualImam->tarikh->format('Y-m-d')) }}" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                            </div>
                            <div>
                                ...
                                <select name="waktu_solat" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                                    @foreach(['Subuh', 'Zohor', 'Asar', 'Maghrib', 'Isyak', 'Jumaat', 'Tarawih', 'Hari Raya'] as $waktu)
                                        <option value="{{ $waktu }}" {{ old('waktu_solat', $jadualImam->waktu_solat) === $waktu ? 'selected' : '' }}>{{ $waktu }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="status" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                                    @foreach(['Dijadual', 'Selesai', 'Ganti', 'Batal'] as $status)
                                        <option value="{{ $status }}" {{ old('status', $jadualImam->status) === $status ? 'selected' : '' }}>{{ $status }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Nama Ganti (jika ada)</label>
                                <input type="text" name="nama_ganti" value="{{ old('nama_ganti', $jadualImam->nama_ganti) }}" class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-700 mb-1">Catatan</label>
                                <textarea name="catatan" rows="2" class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">{{ old('catatan', $jadualImam->catatan) }}</textarea>
                            </div>
                        </div>
                    </div>
