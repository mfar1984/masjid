...
<head>
    ...
    <title>Derma & Sumbangan - E-Masjid</title>
    <x-favicon />
        ...
        <div class="container mx-auto px-0 py-0">
            <div class="bg-white shadow-lg border-x border-gray-200 p-6">
                ...
                <div class="mb-6 flex items-center justify-between">
                    <div>
                        <h1 class="text-xl font-bold text-gray-900 mb-1">Derma & Sumbangan</h1>
                        <p class="text-xs text-gray-600">Rekod derma dan sumbangan</p>
                    </div>
                ...
                <form method="POST" action="{{ route('kutipan-dana.store') }}" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="jenis_kutipan" value="Derma & Sumbangan">

                    <!-- Section 1: Maklumat Penderma -->
                    <div class="bg-green-50 rounded-lg p-4 mb-6">
                        <h2 class="text-sm font-semibold text-gray-900 mb-4">Maklumat Penderma</h2>
                        
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label for="nama_penderma" class="block text-xs font-medium text-gray-700 mb-2">Nama Penderma *</label>
                                <input type="text" id="nama_penderma" name="nama_penderma" value="{{ old('nama_penderma') }}" required maxlength="255" class="w-full px-3 py-2 border border-gray-300 rounded-sm text-xs">
                                @error('nama_penderma')<span class="text-xs text-red-600">{{ $message }}</span>@enderror
                            </div>
                            ...
                            <div>
                                <label for="jenis_derma_id" class="block text-xs font-medium text-gray-700 mb-2">Jenis Derma *</label>
                                <select id="jenis_derma_id" name="jenis_derma_id" required class="w-full px-3 py-2 border border-gray-300 rounded-sm text-xs">
                                    <option value="">-- Pilih Jenis Derma --</option>
                                    @foreach($jenisDerma as $jenis)
                                        <option value="{{ $jenis->id }}" {{ old('jenis_derma_id') == $jenis->id ? 'selected' : '' }}>
                                            {{ $jenis->nama_kategori }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('jenis_derma_id')<span class="text-xs text-red-600">{{ $message }}</span>@enderror
                            </div>

                            <div>
                                <label for="tarikh_kutipan" class="block text-xs font-medium text-gray-700 mb-2">Tarikh Derma *</label>
                    ...
                    <div class="bg-green-50 rounded-lg p-4 mb-6">
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            <div>
                                <label for="tujuan_derma" class="block text-xs font-medium text-gray-700 mb-2">Tujuan Derma</label>
                                <textarea id="tujuan_derma" name="tujuan_derma" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-sm text-xs">{{ old('tujuan_derma') }}</textarea>
                                @error('tujuan_derma')<span class="text-xs text-red-600">{{ $message }}</span>@enderror
                            </div>
