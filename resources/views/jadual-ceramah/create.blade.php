...
<head>
    ...
    <title>Tambah Jadual Ceramah - E-Masjid</title>
    <x-favicon />
        ...
        <div class="container mx-auto px-0 py-0">
            <div class="bg-white shadow-lg border-x border-gray-200 p-6">
                <div class="flex items-center mb-6">
                    ...
                    <div>
                        <h1 class="text-xl font-bold text-gray-900 mb-1">Tambah Jadual Ceramah</h1>
                        <p class="text-xs text-gray-600">Daftar jadual ceramah baru</p>
                    </div>
                </div>
                <form action="{{ route('jadual-ceramah.store') }}" method="POST">
                    @csrf
                        ...
                        <div class="bg-gray-50 p-4 rounded-md border border-gray-200">
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <select name="penceramah_id" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                                        ...
                                        @foreach($penceramahList as $penceramah)
                                            <option value="{{ $penceramah->id }}" {{ old('penceramah_id') == $penceramah->id ? 'selected' : '' }}>{{ $penceramah->nama }}</option>
                                        @endforeach
                                    </select>
                                    @error('penceramah_id')<span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-1">Tajuk Ceramah <span class="text-red-500">*</span></label>
                                    <input type="text" name="tajuk_ceramah" value="{{ old('tajuk_ceramah') }}" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                                    @error('tajuk_ceramah')<span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                                </div>
                                <div>
                                    ...
                                    <select name="jenis_ceramah" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                                        @foreach(['Kuliah Subuh', 'Kuliah Maghrib', 'Kuliah Isyak', 'Ceramah Jumaat', 'Ceramah Khas', 'Tazkirah', 'Lain-lain'] as $jenis)
                                            <option value="{{ $jenis }}" {{ old('jenis_ceramah') === $jenis ? 'selected' : '' }}>{{ $jenis }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="lokasi" value="{{ old('lokasi') }}" placeholder="cth: Dewan Utama" class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                                </div>
                            </div>
                        </div>
                        <!-- Maklumat Bayaran -->
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <select name="jenis_bayaran" id="jenis_bayaran" required class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                                        @foreach(['Sekali', 'Mingguan', 'Bulanan', 'Percuma'] as $jenis)
                                            <option value="{{ $jenis }}" {{ old('jenis_bayaran', 'Sekali') === $jenis ? 'selected' : '' }}>{{ $jenis }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div id="bayaran_fields">
                                    <div>
                                        ...
                                        <input type="number" name="kadar_bayaran" value="{{ old('kadar_bayaran', 0) }}" step="0.01" min="0" class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                                    </div>
                                </div>
                                <div class="border-t pt-4 mt-4">
                                    ...
                                    <div class="space-y-3">
                                        <div>
                                            ...
                                            <input type="number" name="kos_pengangkutan" value="{{ old('kos_pengangkutan') }}" step="0.01" min="0" class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-gray-700 mb-1">Kos Penginapan (RM)</label>
                                            <input type="number" name="kos_penginapan" value="{{ old('kos_penginapan') }}" step="0.01" min="0" class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-gray-700 mb-1">Kos Makan Minum (RM)</label>
                                            <input type="number" name="kos_makan_minum" value="{{ old('kos_makan_minum') }}" step="0.01" min="0" class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-gray-700 mb-1">Kos Lain (RM)</label>
                                            <input type="number" name="kos_lain" value="{{ old('kos_lain') }}" step="0.01" min="0" class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-medium text-gray-700 mb-1">Catatan Kos</label>
                                            <textarea name="catatan_kos" rows="2" class="w-full px-3 py-2 text-xs border border-gray-300 rounded-md">{{ old('catatan_kos') }}</textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
